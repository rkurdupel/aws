...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(max_length=100, unique=True, default=None)    # email field checks whether the email is valid
    ...
    def create(email, password, first_name=None, middle_name=None, last_name=None,  role='0'):
        ...
        """
        role = 'librarian' if role == '1' else 'visitor'

        if len(first_name) <= 20 and len(middle_name) <= 20 and len(last_name) <= 20 and len(email) <= 100 and len(
                email.split('@')) == 1 and len(CustomUser.objects.filter(email=email)) == 0:
